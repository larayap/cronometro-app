...
## 2026-08-09 | init | windows-autostart-single-instance | `sdd-init` corrió sobre un worktree recién creado sin `memory/` previo (ADR-0009: `/memory` está gitignoreado en la raíz del repo, así que `git worktree add` no copia el `memory/` acumulado del repo principal — solo trackea archivos versionados). `_profile.md` se generó desde cero escaneando los archivos reales de este worktree (package.json, vue.config.js, babel.config.cjs, jsconfig.json, .eslintrc.js, .nvmrc, .github/workflows/, src/background.js, src/main/). El conocimiento acumulado en `/home/larayap/cronometro-app/memory/` (14 ADRs, specs de capacidades previas) no quedó disponible en este worktree; queda como gap para fases posteriores del pipeline (`sdd-explore`/`sdd-design`) si necesitan ese contexto histórico — deberán re-derivarlo del código real o el orquestador deberá inyectarlo explícitamente.
2026-08-09 | orchestrator | windows-autostart-single-instance | Corpus specs/adrs sincronizado desde $REPO/memory (gitignoreado) al worktree para resolución de deltas
2026-08-09 | orchestrator | windows-autostart-single-instance | Oráculo C2 retornó not-high-risk pese a riesgos con "probabilidad: Alta" en la prosa de proposal.md (el parse canónico no los contabiliza como proposal.risks estructurado). Se sigue el camino único del oráculo per ADR-0018; sin override del usuario (--require-judgment no provisto).
2026-08-09 | orchestrator | windows-autostart-single-instance | Desviación documentada del Paso 8 de sdd-archive: memory/ está gitignoreado en este repo (decisión previa del proyecto), no existe git rm/cleanup commit posible; en su lugar el conocimiento (specs, adrs, observations, workspace del cambio) se sincroniza al memory/ del repo principal antes del worktree remove.

2026-08-09 | envelope-deviation | ui-branding-polish | sdd-init declaró `/home/larayap/cronometro-app/.sdd/worktrees/ui-branding-polish/memory/changes` (directorio) en §E "Artefactos producidos"; el assert genérico verifica archivos con `test -f`. Verificado con `test -d` y aceptado: el artefacto existe, la desviación es de formato del envelope, no un artefacto faltante.

## 2026-08-09 | debt-candidate | CSS muerto en `src/history/TitleBar.vue`
**Detectado por**: sdd-explore en `ui-branding-polish`
**Ubicación**: `src/history/TitleBar.vue:85-94` (regla `.modal-content { ...; outline: none; }`)
**Descripción**: el `<style scoped>` de este archivo declara `.modal-content`/`.modal-content h3`/`.modal-content ul`/`.modal-content li`, pero el `<template>` de este componente (barra de título de la ventana de historial, solo minimizar/cerrar) no contiene ningún elemento con esas clases — no hay modal en este archivo. Es CSS copiado del `TitleBar.vue` de la ventana principal (que sí usa un modal para la lista de apps) y nunca removido. Inocuo en runtime (regla sin selector que matchee), pero confunde a cualquiera que la lea buscando dónde se suprime el foco de un modal real.
**Promoción sugerida**: incluir en un futuro `sdd new css-cleanup --domain debt` (o corregir inline si `ui-branding-polish` ya toca este archivo por el ítem 2 del foco).

## 2026-08-09 | debt-candidate | Doble mecanismo de toggle de widgets con rótulos inconsistentes
**Detectado por**: sdd-explore en `ui-branding-polish`
**Ubicación**: `src/components/Menu.vue:70-74` (tarjetas "M"/"A"/"P") vs `src/components/TitleBar.vue:4-27` (franja `toggle-squares`, "Manual" con ícono / "W" / "P")
**Descripción**: dos superficies de UI distintas (la pantalla "Inicio" de `Menu.vue` y la franja de la barra de título) controlan exactamente el mismo estado (`appStore.toggleOption`, store `menu.js`) para activar/desactivar los mismos tres widgets, pero usan rótulos distintos para el mismo módulo: `Menu.vue` lo llama "A" (de "Aplicación"), `TitleBar.vue` lo llama "W" (de "Work"). Preexistente al cambio, no nombrado por el Intent (que solo pide renombrar el `<h1>` del módulo, ítem 5). Si `sdd-propose` no decide explícitamente extender el rename a este rótulo, la inconsistencia "A" en una pantalla vs "W"/futuro ícono en la otra sobrevive sin cambios.
**Promoción sugerida**: decidir alcance dentro de `ui-branding-polish` ítem 5, o `sdd new unify-widget-toggle-labels --domain debt` si queda fuera.

## 2026-08-09 | pre-adr | Primera capa de estilos compartida del proyecto (`src/styles/tokens.css`)
**Detectado por**: sdd-propose en `ui-branding-polish`
**Descripción**: la propuesta decide introducir `src/styles/tokens.css` con un `:root` que declara **solo** los colores nuevos del cambio (anillo de foco y grises del rango del calendario), importado por separado en `src/main.js` y `src/history/main.js`. Es el primer punto único de declaración de color del proyecto (hoy cada componente hardcodea sus hex). La restricción de forma no es negociable: CSS puro, porque el bundle de historial no monta Pinia (ADR-0012) — precedente ya existente en ese entry: `import 'v-calendar/style.css'`. La decisión NO incluye migrar los hex ya existentes. Si `sdd-design` la formaliza, es material de ADR (introduce una capa de estilos transversal a dos bundles).

## 2026-08-09 | scope-decision | Rótulos "A" vs "W" y `"Days of Work"` quedan fuera de `ui-branding-polish`
**Detectado por**: sdd-propose en `ui-branding-polish`
**Descripción**: resolución de las dos decisiones de alcance que `sdd-explore` dejó abiertas. (1) `Menu.vue` no se iconifica: hacerlo obligaría a decidir también la tarjeta de Manual, que el Intent no nombra; la inconsistencia "A" (Menu) vs ícono (TitleBar) sobrevive, sin empeorar respecto de hoy ("A" vs "W"). El candidato `unify-widget-toggle-labels` sigue vigente. (2) `HistoryView.vue:5` (`"Days of Work"`) queda fuera del rename: ahí "Work" es el sustantivo común, no el rótulo del módulo, y sustituirlo produce "Days of Work Tracker". Rebrandear ese encabezado exige un string distinto — decisión de copy, no de rename.

## 2026-08-09 | architecture | ADR-0015: capa de tokens CSS compartida por los dos bundles del renderer
**Detectado por**: sdd-design en `ui-branding-polish` (cierra el `pre-adr` que dejó sdd-propose)
**Descripción**: `src/styles/tokens.css` con un `:root` de custom properties **y** la regla transversal que las consume (el anillo de foco `:focus-visible`), importado como primer import de `src/main.js` y de `src/history/main.js`. CSS puro por obligación: el bundle de historial no monta Pinia (ADR-0012) y el precedente de importar una hoja suelta ya existe en ese entry (`v-calendar/style.css`). Cuatro reglas del patrón: CSS puro sin excepción; un import explícito por entry; solo entra lo compartido (los hex ya hardcodeados **no** se migran); nombres semánticos kebab-case en `:root` sin prefijo de proyecto. Completa el trío de decisiones sobre la frontera entre bundles: ADR-0010 (qué código no cruza), ADR-0012 (cómo cruza el dato de preferencia), ADR-0015 (cómo cruza la constante de presentación). La alternativa de dejar solo variables en el archivo y duplicar la regla en `App.vue` + `HistoryView.vue` se descartó porque reintroduce un nivel más arriba la duplicación que el archivo existe para evitar.

## 2026-08-09 | gap | El inventario de foco de `exploration.md` está incompleto: 9 `outline: none` y 4 focables más
**Detectado por**: sdd-design en `ui-branding-polish`
**Descripción**: `exploration.md` lista 7 declaraciones `outline: none` y ~12 controles de foco nativo ("19 selectores"). El grep real sobre el worktree devuelve **9** `outline: none`: faltan `OpcionesPanel.vue:81` (`.modal-content`, contenedor sin `tabindex`, inerte) y `CronometroPomodoro.vue:330` (`.edit-input`, un `<input type="number">` **real y focable** que hoy queda sin ninguna señal de foco). El barrido de `<input|<select|<button|tabindex` agrega además cuatro controles focables que la tabla no nombra: `AppRow.vue:12` (nombre de sesión), `CronometroAplicacion.vue:54` (`.group-name-input`), `CronometroPomodoro.vue:17` (`.remove-btn`) y el `.close-btn` del modal de `TitleBar.vue:67`. **Lección de método**: un checklist de selectores enumerado a mano no sirve como criterio de aceptación de un barrido de foco. El diseño reemplaza el conteo por una regla global más tres invariantes verificables por grep (`design.md § D-1.4`).

## 2026-08-09 | debt-candidate | Override de v-calendar inerte en `HistoryView.vue`: `.vc-blue` no va en la capa de fondo
**Detectado por**: sdd-design en `ui-branding-polish`
**Ubicación**: `src/history/HistoryView.vue:277-280` (`.dark-calendar .vc-highlight.vc-highlight-bg-solid.vc-blue`)
**Descripción**: `CalendarDay` de v-calendar 3 renderiza cada highlight como dos elementos anidados (`dist/es/index.js:4904-4913` y `6904-6916`): la clase de color (`vc-blue`) va en el **wrapper** `.vc-attr` y en el **contenido** del día, nunca en el `.vc-highlight` de la capa de fondo. Esa regla, por lo tanto, no matchea ningún elemento: el gris del día seleccionado lo produce en realidad la regla vecina, la del contenido (`.vc-day-content.vc-highlight-content-solid.vc-blue`). **No se corrige en este cambio**: cambiarla alteraría la apariencia del día puntual, que `range-selection-grayscale-palette` congela explícitamente ("conserva su apariencia gris actual, sin cambios"). El riesgo inmediato que sí importa es que copiar ese patrón para el rango produciría un override silenciosamente inerte — el diseño lo evita omitiendo `.vc-blue` en los selectores de capa de fondo (`design.md § 0.3` y `§ D-2`).
**Promoción sugerida**: incluir en un futuro `sdd new css-cleanup --domain debt`, junto con el bloque muerto de `history/TitleBar.vue:72-124`.

## 2026-08-09 | gap | Medidas reales de los assets de ícono: el riesgo de la propuesta estaba invertido
**Detectado por**: sdd-design en `ui-branding-polish` (Pillow sobre los archivos de origen)
**Descripción**: `work_worktracker.webp` es **567×562** (apaisado), no 562×567 (retrato) como declaran `exploration.md` y `proposal.md`; `pomodoro_worktracker.webp` sí es 617×681. El bounding box del canal alfa cubre el lienzo completo en los dos: no tienen márgenes transparentes. `src/assets/manual.png`, en cambio, es 32×32 con bbox `(4,0,28,32)` — 4px transparentes a cada lado. En una caja de 24×24 con `object-fit: contain` el ancho de arte visible resulta ≈24px (work), ≈21.7px (pomodoro) y ≈18px (manual): los íconos nuevos se ven **iguales o más grandes** que el existente, no más chicos. El riesgo "quedan ópticamente más chicos que `manual.png`" queda desestimado por medición y el diseño no aplica ninguna compensación de entrada.

## 2026-08-09 | scope-decision | Tres archivos declarados en `scope[]` de specs quedan de solo lectura
**Detectado por**: sdd-design en `ui-branding-polish`
**Descripción**: resolución de la inconsistencia entre `scope[]` y el alcance aprobado, para que `sdd-tasks` no genere tareas de más ni `sdd-verify` lea omisiones. (1) `vue.config.js` (en `scope[]` de `app-icon-matches-pomodoro-artwork`): su `win.icon` ya apunta a `public/icon-work-256.png`, el archivo que el cambio regenera **en sitio con el mismo nombre y dimensiones**, así que no hay nada que editar — `proposal.md` tiene razón y la verificación que corresponde es afirmativa y de lectura. (2) `src/components/Menu.vue` (en `scope[]` de `keyboard-focus-indicator-per-palette`): su `.apply-btn` queda cubierto por la regla global y el archivo no tiene ningún `outline: none`. (3) `src/stores/monitoredApps.js` (en `scope[]` de `added-apps-review-tab`): la pestaña nueva consume `selection`, `icons`, `ensureIcons` y `removeApp`, que ya existen con la forma necesaria. Criterio general: `scope[]` enumera los archivos que **gobiernan** la capacidad, no los que un cambio modifica. Las specs no se editan por esto (un `scope[]` amplio no es un error de comportamiento y un delta es para evolución de conducta, no para metadata).

## 2026-08-09 | scope-decision | Rótulos de las tres pestañas del selector: "Procesos abiertos" pasa a "Abiertas"
**Detectado por**: sdd-design en `ui-branding-polish`
**Descripción**: los tres rótulos quedan "Instaladas / Abiertas / Agregadas" — tres adjetivos femeninos plurales que concuerdan con "aplicaciones", el sujeto del título del modal. Renombrar la pestaña existente es un cambio de copy que el Intent no pide explícitamente, y se toma porque con tres pestañas en 260px de contenido la fila necesita rótulos paralelos: conservar "Procesos abiertos" lo obliga a partirse en dos líneas y, con `flex: 1`, engorda las tres pestañas por igual. Sin conflicto con `installed-apps-data-integrity`: esa spec especifica el **comportamiento** de esa vista (vía de alta alternativa para programas sin acceso directo reconocible), que no cambia. El estilo acompaña con `font-size: 0.8rem` y `white-space: nowrap` en `.tabs button`.

## 2026-08-09 | ux-nit | El toggle "Agregar como" queda visible en la pestaña donde el clic elimina
**Detectado por**: sdd-design en `ui-branding-polish`
**Ubicación**: `src/components/AppSelectorModal.vue:26-40` (`.type-toggle`)
**Descripción**: en la pestaña "Agregadas" toda entrada está seleccionada, así que el clic siempre da de baja; el toggle "Agregar como: Permanente / Solo esta vez" que queda arriba gobierna el próximo **alta**, no lo que esa vista hace. Se deja visible a propósito: `proposal.md § Scope` fija que buscador, toggle de tipo y aviso de límite "quedan tal como están", esconderlo con un `v-if` haría saltar el layout al conmutar de pestaña, y el toggle sigue siendo válido para el alta que el usuario haga al volver a otra pestaña (el aviso de límite es incluso más útil ahí, porque explica por qué convendría quitar una). Queda anotado como fricción menor, no corregido.

2026-08-09 | env-constraint | ui-branding-polish | El worktree se creó sin `node_modules` (el repo principal los tiene, 711M) y su `package.json` es idéntico al de main, así que el orquestador enlazó `node_modules` por symlink al del repo principal para habilitar `npm run lint` en el worktree. Verificado: ESLint 7.32.0 responde. El symlink no aparece en `git status` (cubierto por `.gitignore`).

2026-08-09 | env-constraint | ui-branding-polish | **`electron:build` y `electron:serve` no son ejecutables en este entorno.** El único Node instalado es v24.15.0 (`~/.nvm/versions/node/` solo tiene esa versión) y `.nvmrc` fija 16.20.2. El webpack 4.47.0 anidado del main process hashea con `md4`, que OpenSSL 3 rechaza en Node >=17, y Node 24 ya no acepta `--openssl-legacy-provider` como escape. Consecuencia: el techo sintáctico ES2016 del bundle del main NO se puede validar compilando en este entorno. La verificación de ese invariante para `src/background.js` queda en chequeo estático (grep de `??` y `?.` sobre el archivo y sus alcanzables), y la compilación real queda diferida al entorno Windows ya montado. Esto afecta el criterio de cierre del ítem 7 y la tarea de `background.js`.

## 2026-08-09 | deferred-windows | Dos acceptance criteria de `app-icon-matches-pomodoro-artwork` quedan sin marcar tras `sdd-apply`
**Detectado por**: sdd-apply en `ui-branding-polish` (Tarea 6.4)
**Descripción**: de los cuatro acceptance criteria de la spec, los dos primeros (ícono de ventana, ícono de bandeja) se verificaron inspeccionando con Pillow los dos PNG regenerados (`public/icon-work-256.png` 256×256 RGBA, `public/img/icon-work.png` 32×32 RGBA, ambos con el arte de Pomodoro paddeado a cuadrado transparente sin recorte) y confirmando que `src/background.js` los lee en las rutas correctas. Los otros dos —el ícono embebido en el instalador `.exe` NSIS y el ícono del acceso directo que ese instalador crea— exigen compilar el paquete de Windows (`electron-builder` con `target: 'windows-latest'`), que este WSL2 no ejecuta (ver el `env-constraint` anterior sobre `electron:build`/`electron:serve`). La spec misma lo declara en su `## Observations`, agregada en `sdd-design`. Quedan explícitamente **sin marcar** en `app-icon-matches-pomodoro-artwork.md` tras esta fase — junto con el cuarto criterio ("el arte se ve completo, sin recorte"), que la propia spec agrupa con estos dos ("los dos últimos") porque abarca los cuatro contextos de ícono, no solo los dos verificables acá.
**Promoción sugerida**: ninguna — la verificación queda diferida a `sdd-verify`/al entorno de build de Windows ya montado (`_profile.md`), dentro del ciclo normal de este mismo cambio, no como deuda separada.

## 2026-08-09 | verification-gap | ui-branding-polish | sdd-verify auditó los 9 commits contra las 8 specs — PARTIAL por limitación de entorno, sin ningún defecto de código detectado
**Fase**: sdd-verify
**Contexto**: se auditó el diff completo (`532030c..HEAD`, 17 archivos) contra `design.md`, `tech-context.md` y las 8 specs de `spec_refs`. `npm run lint -- --no-fix` (0 errores, 1 warning preexistente), `npm run build` (renderer limpio) y `grep -n "??\|?\." src/background.js` (sin resultados) pasan en verde. Las 9 supresiones de `outline: none` coinciden línea a línea con D-1.2; la excepción `.modal-content:focus` de `TitleBar.vue` usa el token y `outline-offset` negativo; las 4 reglas nuevas del rango en `HistoryView.vue` evitan correctamente el patrón inerte de `.vc-blue` sobre la capa de fondo (D-2/§0.3); `addedApps()` filtra `monitoredApps.selection` (no `installedApps`) y `choose()` conserva el guard `isSelected` antes de `limitReached`; el slot `#header` de `vuedraggable` se aplicó en los dos `<draggable>` de grupo sin tocar `dragUngrouped` ni ningún handler/guarda de arrastre; los dos PNG regenerados (256×256 y 32×32, ambos RGBA con el alfa llenando el lienzo) coinciden con D-7. Las exclusiones de `proposal.md § Scope` (vue.config.js, Menu.vue, monitoredApps.js, "Days of Work", sin .ico, sin IPC) se confirmaron todas con diff vacío. El grafo de las 8 specs (depends_on/affects, y las dos parejas supersedes/superseded_by con `unified-product-identity` y `group-composition-and-drag`) es simétrico, sin WARN ni corrección de metadata necesaria.
**Veredicto**: PARTIAL — causa explícita: limitación de entorno (sin `npm run electron:serve`, sin compilación NSIS de Windows), no defecto de implementación. `verified_at` de las 8 specs queda en `null` por instrucción explícita del dispatch (solo se actualiza en PASS).
**Nota metodológica menor**: el invariante literal `grep -rn ":focus-visible" src/` → "una sola línea" de `design.md`/`tasks.md` da 4 líneas, no 1, porque 3 son prosa de comentarios que el propio diseño manda literal (no reglas CSS duplicadas). `grep -n "^:focus-visible\s*{" src/` confirma una sola declaración real. No es un defecto; se documenta para que futuras verificaciones no lean la discrepancia como regresión.
**Acción sugerida**: retomar en el entorno Windows ya montado (`_profile.md`) los checklists `[manual]` de `tasks.md` (recorrido de foco, rango, pestaña "Agregadas", los 4 flujos de arrastre, y el ícono embebido del instalador NSIS/acceso directo). Detalle completo en `changes/ui-branding-polish/verify-report.md`.

## 2026-08-09 — sdd-judgment de `ui-branding-polish`: escalated tras 2 iteraciones

**[judgment]** Dos iteraciones, cuatro jueces Opus independientes, revisión estática (el entorno
no renderiza la app; `verify_verdict: PARTIAL-por-entorno` anotado por el orquestador habilitó el
dispatch). Veredicto: `escalated` con 1 hallazgo confirmado vivo y 2 suspect `low`. Detalle
completo en `changes/ui-branding-polish/judgment-report.md`.

**[a11y]** Patrón a recordar: **una regla global `:focus-visible` no garantiza indicador de foco
en todo control focable**, y `design.md` de este cambio lo asumió. Falla por dos vías distintas,
una detectada por cada juez de la iteración 2:
1. **Cascada de terceros**: `v-calendar` declara `.vc-container *:focus { outline: none }` a nivel
   superior (0,2,0) y le gana a `:focus-visible` (0,1,0). Dentro del calendario solo quedan
   cubiertos los elementos que la librería marca con `.vc-focus` — y el botón de mes/año
   (`class: "vc-title"`, `dist/es/index.js:6239`) **no la lleva**, así que queda sin ningún
   indicador. Al integrar un indicador de foco global con una librería que suprime el outline,
   hay que enumerar los focables de la librería, no confiar en la regla global.
2. **Contraste sobre fondo dinámico**: un anillo claro (`#e0e0e0`) con `outline-offset` positivo
   se dibuja sobre el fondo del *contenedor*, no del control. Si ese fondo lo pinta un estilo
   inline (`getFillStyle` de `CronometroPomodoro.vue` usa `#d3d3d3` para sesiones completadas),
   el anillo cae a 1.13:1. Un token único de foco no sirve para paletas mixtas: hace falta
   acotar el color por componente donde el fondo se aclara.

**[css]** `min-height` sobre el nodo raíz de un `<draggable>` de `vuedraggable`: mover contenido
al slot `#header` lo mete **dentro** de esa caja, así que el `min-height` deja de expresar "área
de destino" y pasa a expresar "área total". Sin subirlo, el bloque se encoge (73px → 42px en este
caso). Corregido en `0cfd1e7`. `design.md § D-6.4` había dado por sentado que el modelo de caja
era idéntico: cierto para el bloque cuya altura la fija su contenido, falso para el que la fija un
`min-height` sobre el propio nodo Sortable.

**[sortablejs]** Dos mecánicas del código instalado, útiles para cualquier cambio futuro sobre el
drag & drop: (a) el slot `#header` de `vuedraggable` **no** recibe `data-draggable`, y por eso no
arranca gestos (`_onTapStart` resuelve `target = null`), no intercepta `mousedown` —un `<input>`
adentro sigue operable— y la traducción índice-DOM ↔ índice-modelo de `getVmIndexFromDomIndex` lo
ignora; (b) pero **sí** desactiva el `emptyInsertThreshold` de esa lista, porque
`_detectNearestEmptySortable` (`sortable.core.esm.js:1106-1110`) aborta cuando
`lastChild(sortable)` es truthy y lo consulta **sin selector**: una lista con header ya no es
"vacía" para el halo de 5px.

**[pre-adr]** `var(--token)` sin valor de respaldo no cae a un color por defecto: la declaración
queda *invalid at computed-value time* y se comporta como `unset` — `outline` cae a
`outline-style: none`. Para un token de foco eso significa **ningún indicador, en silencio**.
ADR-0015 documentaba lo contrario; corregido en su regla 2 de `§ Decision`. Decisión de triage:
**no** agregar respaldos en los tres puntos de consumo (duplicaría el valor del token contra el
SSOT que el propio ADR establece, y la protección sería parcial porque los tres tokens del rango
tampoco lo llevan). La mitigación elegida es documental.

2026-08-09 | user-decision | ui-branding-polish | `sdd-judgment` escaló con el hallazgo confirmado C-1 vivo (dos instancias: `.vc-title` sin ningún indicador de foco por especificidad de `v-calendar`, y anillo `#e0e0e0` a 1.13:1 sobre el fondo `#d3d3d3` de una sesión completada en Pomodoro) más dos suspect `low` de limpieza en `TitleBar.vue`. El orquestador presentó `judgment-report.md` al usuario con sus tres opciones. **El usuario eligió corregir I-1 e I-2 y cerrar**, incluida la limpieza de los dos suspect. El pipeline se reanuda con un delta de `keyboard-effect-indicator-per-palette` (la spec cuyo requisito absoluto quedó incumplido) → sdd-apply → sdd-verify → sdd-archive.

2026-08-09 | orchestrator-override | ui-branding-polish | El orquestador persistió `skip_judgment: true` en `state.md`. Justificación: `proposal.effort` es `L`, así que el oráculo C2 `post-verify-router` volvería a rutear a `sdd-judgment` en el `sdd-verify` de cierre; pero `sdd-judgment` ya consumió las 2 iteraciones que su protocolo admite y el usuario tomó la decisión de cierre con el `judgment-report.md` completo en mano. Sin el override, el pipeline no termina. `skip_judgment` tiene precedencia máxima por diseño (`sdd-phase-common § High-risk change — Override manual del usuario`), y este es exactamente su caso de uso: una decisión humana informada que releva el trigger automático.

## 2026-08-09 | judgment-fix | ui-branding-polish | sdd-spec emitió el delta MODIFY de `keyboard-focus-indicator-per-palette` ordenado por el usuario tras la escalada de `sdd-judgment`
**Fase**: sdd-spec
**Contexto**: la spec nueva `judgment-fixes-ui-branding-polish-round2` (capability `focus-indicators`) supersede a `keyboard-focus-indicator-per-palette` y cubre I-1/I-2 (comportamiento: `.vc-title` sin indicador propio por cascada de `v-calendar`; anillo imperceptible sobre el fondo de una sesión Pomodoro completada) más S-1/S-2 (limpieza en `TitleBar.vue`, marcados explícitamente como `SHOULD` sin comportamiento observable, sin inflarlos). `Requirements`/`Scenarios` quedaron en lenguaje de negocio puro; los datos técnicos (especificidad 0,2,0 vs 0,1,0, contraste 1.13:1, líneas exactas) se movieron a `## Observations` y los archivos a `scope[]`, por instrucción explícita del dispatch.
**Decisión de `adrs[]`**: se incluyó ADR-0015 (gobierna cómo cruza una constante de presentación entre bundles — exactamente lo que esta corrección toca) y se excluyó deliberadamente ADR-0012 (gobierna el cruce del *dato* de preferencia vía IPC sin Pinia): la corrección es CSS puro dentro de un `<style>` ya existente de `HistoryView.vue`, no introduce IPC ni store, así que no cruza la superficie que ADR-0012 regula. El dispatch sugería evaluar ADR-0012 condicionalmente ("si el delta cruza al bundle de historial") — el análisis concluyó que no aplica.
**Simetría del grafo**: `shared-accent-colors-across-windows.md` (de quien el nuevo delta depende) se actualizó agregando el nuevo slug a `related[]`/`affects[]` (sin remover la referencia al slug superseded, siguiendo el patrón ya presente en el corpus de mantener referencias históricas a specs superseded — ver `unified-product-identity`/`group-composition-and-drag` referenciadas desde varias specs vigentes). `spec_refs` de `state.md` reemplazó el slug superseded por el nuevo delta, siguiendo el mismo patrón que `full-surface-drop-target-for-grouping` reemplazó a `group-composition-and-drag`.
**Acción sugerida**: ninguna — el pipeline continúa a `sdd-apply` con `current_phase` ya actualizado.

## 2026-08-09 | judgment-fix | ui-branding-polish | sdd-apply implementó I-1/I-2/S-1/S-2 de `judgment-fixes-ui-branding-polish-round2`, con evidencia de cascada y contraste verificada por lectura de código y build, no asumida
**Fase**: sdd-apply

**I-1 — especificidad verificada, no asumida**: re-confirmé por grep directo sobre
`node_modules/v-calendar/dist/style.css:1264-1266` (independiente del cálculo ya hecho por
`sdd-judgment`) que `.vc-container:focus, .vc-container *:focus { outline: none }` está
declarada a nivel superior de la hoja, sin `@media`, y que no existe ninguna regla
`focus-visible` en todo ese archivo (`grep -c focus-visible` → 0). Especificidad máxima de
esa regla: (0,2,0). La regla agregada, `.dark-calendar .vc-title:focus-visible`, calcula
(0,1,0)+(0,1,0)+(0,1,0) = (0,3,0) — gana por especificidad sola, sin depender del orden de
inyección entre hojas. Verificado además en el bundle compilado tras `npm run build`:
`dist/css/history.*.css` contiene la regla nueva; `dist/history.html` confirma que
`chunk-vendors.css` (v-calendar) se enlaza **antes** que `history.css` en producción — el
mismo orden invertido que `judgment-report.md` nota #1 ya había señalado sobre `design.md
§ D-2.1` — así que aun en el caso hipotético de empate de especificidad, la regla del
proyecto también gana por orden de cascada. Doble margen, no solo uno.

**I-2 — el ejemplo del triage (`#1b1b1b`) no sirve tal cual; se calculó un color que
funciona en los tres estados**: `judgment-report.md` sugiere `outline-color: #1b1b1b` como
dirección de corrección. Antes de aplicarlo, calculé el contraste real (fórmula WCAG,
luminancia relativa) contra los dos fondos sólidos que `.edit-input`/`.remove-btn` pueden
tener detrás (`getFillStyle()`: `black` por defecto/pendiente, `#d3d3d3` completada — el
"degradado" de la sesión en curso es en realidad un corte sólido sin mezcla, `linear-
gradient(to right, #d3d3d3 {p}%, black {p}%)` con el mismo `{p}%` en los dos color-stops,
así que solo hay dos colores de fondo posibles, nunca un intermedio). `#1b1b1b` da 11.51:1
contra `#d3d3d3` pero **1.22:1 contra `black`** — hubiera resuelto I-2 sobre la sesión
completada y roto el mismo requisito sobre la sesión pendiente, que es exactamente el tipo
de defecto que esta corrección busca eliminar. Elegí `#6f6f6f` (ya usado en el propio
proyecto como color de pestaña activa en `HistoryView.vue`): 4.18:1 contra `black`, 3.36:1
contra `#d3d3d3`, ambos ≥ 3:1 (WCAG 2.2 SC 1.4.11, el mismo umbral que `design.md § D-2.3`
adopta). Verificado también en el bundle compilado: `dist/css/index.*.css` contiene
`.edit-input[data-v-3f8a43c3]:focus-visible,.remove-btn[data-v-3f8a43c3]:focus-visible
{outline-color:#6f6f6f}`.

**Lint**: `npm run lint -- --no-fix` → 0 errores, 1 warning preexistente
(`vue/no-deprecated-destroyed-lifecycle`, `CronometroManual.vue:76`), sin tocar ese archivo.
Nota operativa: `npm run lint` sin `--no-fix` autoaplica el fix de esa regla sobre
`CronometroManual.vue` (cambia `beforeDestroy` → `beforeUnmount`) porque el script del
proyecto corre `eslint` con `--fix` por default; revertido con `git checkout` antes de
continuar, porque esa corrección está fuera del alcance de esta tarea. Cualquier `sdd-apply`
futuro que corra `npm run lint` sin el flag en este repo debe esperar el mismo efecto
colateral y revertirlo del mismo modo si el warning sigue fuera de alcance.

**No verificado en esta fase (motivo: limitación de entorno ya documentada)**: el recorrido
real con teclado de los dos Scenarios de comportamiento (I-1, I-2) — exige
`npm run electron:serve`, que no corre en este WSL2. Diferido al recorrido manual en Windows,
igual que el resto del guion de `verify-report.md § Límite de entorno`.

**Acción sugerida**: ninguna — el pipeline continúa a `sdd-verify`.

## 2026-08-09 | judgment-fix | ui-branding-polish | sdd-verify (segunda pasada) auditó de forma independiente la corrección de judgment-fixes-ui-branding-polish-round2
**Fase**: sdd-verify

Re-verifiqué I-1 e I-2 sin confiar en las marcas `[x]` que dejó `sdd-apply`: recalculé a mano
los dos contrastes WCAG de I-2 (3.35:1 vs `#d3d3d3`, 4.18:1 vs `black`, y el `#1b1b1b`
descartado en 1.22:1 vs `black`) y confirmé por lectura directa de `node_modules/v-calendar`
y del bundle compilado propio (`npm run build` corrido de nuevo, no reutilizado) la
especificidad de I-1 y su presencia en `dist/css/history.*.css`. Las cuatro marcas
sobreviven. Sin defecto nuevo, sin exceso de scope, exclusiones de `proposal.md` reconfirmadas.

**Matiz documentado, no defecto**: la nota técnica de I-1 en la spec compara especificidad
contra la regla `outline: none` de v-calendar, pero la corrección usa `box-shadow` — canal
distinto, sin conflicto de cascada real con esa regla. La corrección funciona igual
(confirmado en el bundle), la comparación de especificidad es cierta pero no es la causa
mecánica real. Registrado en `verify-report.md § I-1` para quien edite esto después.

**Hallazgo nuevo de grafo (WARN, no auto-corregido)**: `judgment-fixes-ui-branding-polish`
(fix de la ronda 1 de judgment) declara `depends_on`/`affects` hacia
`full-surface-drop-target-for-grouping`, que no declara la recíproca en `affects` ni
`related`. Preexistente a esta pasada (nadie la reportó porque esa spec no estaba en
`spec_refs` durante la primera pasada de `sdd-verify`). No se auto-corrige: el veredicto de
esta pasada es PARTIAL (no PASS) y la corrección requiere decidir entre `affects` y
`related`, ambigüedad que el protocolo reserva para juicio humano.

**Veredicto**: PARTIAL — limitación de entorno, misma causa exacta que la primera pasada
(sin `npm run electron:serve` en este WSL2). Sin defecto de código en la corrección.
`verified_at` de las 9 specs de `spec_refs` queda en `null`.

**Acción sugerida**: ninguna de código. Recorrido manual en Windows con la guía de
`verify-report.md § Guía de recorrido manual en Windows` (13 puntos, incluye los controles
corregidos en esta ronda). El orquestador decide el siguiente paso con `current_phase: ""`.

## 2026-08-09 | archive | ui-branding-polish | sdd-archive cerró el cambio con tres desviaciones autorizadas por el orquestador

**Fase**: sdd-archive

El dispatch de esta fase autorizó explícitamente tres desviaciones respecto al workflow
estándar de `sdd-archive`. Se registran acá para que queden auditables.

**Desviación 1 — se procedió al archive con veredicto PARTIAL, no PASS.**
`verify-report.md` cierra en PARTIAL por limitación de entorno declarada (WSL2 no puede
correr `npm run electron:serve` ni compilar el instalador NSIS), sin ningún defecto de
código pendiente en ninguna de sus dos pasadas. El oráculo `post-verify-router` del
orquestador retornó `next_phase: sdd-archive` con `reason: skip_judgment` — tratamiento
documentado de PARTIAL-por-entorno como equivalente a PASS para efectos de routing. Se
procedió a los pasos 2–7 sin retornar `blocked`.

**Desviación 2 — el paso 8 (cleanup de `memory/changes/`) fue no-op, no se ejecutó.**
`memory/` está en `.gitignore` de este repositorio desde el commit `532030c` ("chore: keep
the internal SDD corpus out of the public repository") — el corpus SDD nunca estuvo
trackeado en este repo público. `git rm -rf memory/changes/ui-branding-polish/` no aplica
sobre archivos no trackeados, y un commit de cleanup sobre eso sería vacío. No se intentó
el `git rm`, no se forzó nada con `git add -f`, y **no se borró el directorio del
filesystem**: `memory/changes/ui-branding-polish/` no está recuperable vía git en este
repo (a diferencia de lo que asume ADR-0008 para el caso general), y contiene la única
copia de `exploration.md`, `design.md`, `judgment-report.md` y, en particular, la guía de
13 pasos de `verify-report.md § Guía de recorrido manual en Windows` que el usuario
necesita para cerrar los criterios de aceptación diferidos. Borrarlo hubiera sido
irreversible sin respaldo.

**Desviación 3 — el paso 9 (`git worktree remove`) no se ejecutó.**
Por la misma razón que la desviación 2: el worktree es el único lugar donde vive el corpus
completo de este cambio (specs nuevas, ADR-0015, `observations.md`, y los artefactos
listados arriba). El orquestador retiene la decisión sobre el destino del worktree después
de este retorno; `sdd-archive` lo deja intacto.

**Qué sí se ejecutó (pasos 2–7)**: working tree ya estaba limpio (nada que commitear en el
paso 2); push de `feature/ui-branding-polish` a `origin` con `-u` para reapuntar el upstream
(antes apuntaba a `origin/main` por cómo `git worktree add -b` lo creó); PR creado contra
`main` (https://github.com/larayap/work-tracker/pull/7); las 9 specs de `spec_refs` marcadas
`status: completed` con la URL del PR, sin tocar ningún acceptance criterion ni checkbox que
siga dependiendo de verificación en Windows (quedan exactamente como `sdd-apply`/`sdd-verify`
los dejaron); `_profile.md` recibió una entrada nueva en `## Stack` documentando
`src/styles/tokens.css` (ADR-0015) como capa de tokens compartida — se evaluó agregar
también una nota sobre los dos `.webp` nuevos de `src/assets/` y se descartó por YAGNI: son
assets de uso, no un cambio de stack o convención; `state.md` cerrado con
`status: completed`, `current_phase: ""`, `mr_status: created`.

**Acción sugerida**: ninguna. El orquestador decide qué hacer con el worktree y con
`memory/changes/ui-branding-polish/` — este sub-agente no los tocó.

2026-08-09 | orchestrator | ui-branding-polish | **Desviaciones de sdd-archive, decididas por el orquestador y auditables.** (1) Paso 8 (cleanup commit de `memory/changes/`) es NO-OP en este repo: `memory/` está en `.gitignore` (línea 34) desde `532030c` "chore: keep the internal SDD corpus out of the public repository", así que esos archivos nunca estuvieron trackeados y no hay nada que `git rm`. En consecuencia el assert `[ASSERT FAIL] sdd-archive NO creó cleanup commit per ADR-0008` se evalúa como **N/A**, no como fallo: la premisa del assert (que el workspace esté versionado) no se cumple en este repositorio. (2) Paso 9 (`git worktree remove --force`) **omitido**: ADR-0008 trata `memory/changes/` como efímero porque el historial queda recuperable vía git, pero acá no lo está — remover el worktree habría destruido de forma irrecuperable `exploration.md`, `design.md`, `judgment-report.md` y la guía de recorrido manual en Windows de `verify-report.md`, que es justamente lo que el usuario necesita para cerrar los criterios diferidos. (3) Mitigación aplicada: el corpus permanente del cambio (9 specs, ADR-0015, `_profile.md` actualizado) se sincronizó al `memory/` del repo principal con **merge, no sobrescritura** — las 3 specs que el archive de `windows-autostart-single-instance` había dejado en main (`single-instance-focus`, `startup-visibility-preference`, `installer-opt-in-autostart`) se preservaron, y `observations.md` se unió por prefijo común (734 comunes + 4 de main + 257 del worktree = 995). Respaldo previo en el scratchpad de la sesión.

2026-08-09 | orchestrator | ui-branding-polish | `origin/main` avanzó de `532030c` a `9b8bf42` durante este pipeline, por el merge del PR #6 (`windows-autostart-single-instance`, POM-4) más un commit de README — **no** por este cambio. Verificado que los 12 commits de `feature/ui-branding-polish` NO están en `main` y que el PR #7 queda `MERGEABLE`/`CLEAN`: los dos cambios tocaron `src/background.js` en regiones distintas (POM-4 agregó single-instance y visibilidad de arranque; este cambio reapuntó una línea del icono de `BrowserWindow`). Simulación local con `git merge-tree` sin conflictos.

## 2026-09-01 | init | pomodoro-wall-clock-timer | Inicio del cambio (domain: fix, fast_path: apply-only). Corrige el pomodoro de `CronometroPomodoro.vue`, que cuenta ticks de `setInterval` en vez de tiempo real y se ralentiza por intensive wake-up throttling (Chromium 91/Electron 13) en ventana oculta/ocluida — reporte: bloque de 15 min tardó ~30 min reales. Plan: medir restante con `Date.now()` (mismo enfoque que `CronometroManual.vue`) y fijar `backgroundThrottling: false` en `webPreferences` de `mainWindow` en `src/background.js`. Restricción vigente: techo ES2016 del bundle main (sin `??`/`?.`).

## 2026-09-02 | env-quirk | pomodoro-wall-clock-timer | `node_modules` y Node 16.20.2 ausentes en el worktree — instalados en esta pasada

**Detectado por**: sdd-apply en `pomodoro-wall-clock-timer`.
**Descripción**: el worktree no traía `node_modules` (build previo) ni Node 16.20.2 disponible vía `nvm` (el shell traía Node 22 activo). Se corrió `nvm install 16.20.2` (agrega la versión al `nvm` del usuario, no solo a este worktree) y luego `npm ci`, que instaló 1637 paquetes sin bloquear el flujo. `npm run lint -- --no-fix` y `npm run build` corrieron limpio tras eso. `npm run electron:build` no se intentó (memoria del proyecto: wine lo mata desde WSL; T2 solo agrega una propiedad booleana literal, sin sintaxis nueva que ese build pudiera detectar y el renderer build no la evalúa).
**Nota de alcance**: `npm ci` deja `node_modules/` sin trackear (ya estaba en `.gitignore`); no se tocó `package.json`/`package-lock.json`.

## 2026-09-02 | apply | pomodoro-wall-clock-timer | T1 y T2 implementadas, verificadas con lint + build del renderer, commit único

**Fase**: sdd-apply.
**T1** (`src/components/CronometroPomodoro.vue`): se agregó `endAt` (timestamp de fin de bloque) al estado; `start`, `reset`, `nextSession` y `stopEditing` (solo si `running`) lo fijan/reprograman desde `Date.now()`; `pause` recalcula `currentTime` una última vez desde `endAt` antes de limpiar el intervalo; `tick` deriva `currentTime = Math.max(0, Math.ceil((endAt - Date.now()) / 1000))` y limpia el intervalo antes de invocar `nextSession()` para que un disparo tardío repetido no la llame dos veces; `cancel` deja `endAt = null`. No se introdujo `??`/`?.` nuevo (los tres usos preexistentes de `?.` en `reset`/`cancel`/`created` se dejaron intactos — no estaban en alcance de T1 tocarlos, y esa restricción del archivo no es obligatoria para el bundle del renderer).
**T2** (`src/background.js`): `backgroundThrottling: false` agregado a `webPreferences` de `mainWindow`, con comentario en español. Confirmado por grep: cero `??`/`?.` en el archivo.
**Verificación**: `npm run lint -- --no-fix` — 0 errores (1 warning preexistente en `CronometroManual.vue`, no tocado por este cambio). `npm run build` — compila el renderer sin errores (solo warnings preexistentes de tamaño de bundle). No hay test runner en el proyecto (ya documentado en `_profile.md`); la verificación de los criterios de comportamiento (drift bajo tick tardío, pausa/reanudación, edición en caliente, disparo único de `nextSession`) se hizo por revisión del control de flujo contra la especificación de `tasks.md`, no por ejecución de un harness — no se pudo levantar la app (Electron/Windows) en este entorno.
**Commit único** (`c2123ab`) con exactamente `src/background.js` y `src/components/CronometroPomodoro.vue`; `memory/observations.md` y `memory/changes/pomodoro-wall-clock-timer/` quedan sin commitear, como indica la convención de este repo (`memory/` fuera del commit de código).
**No hay specs que actualizar**: `spec_refs` vacío (path `apply-only`).

## 2026-09-02 | debt-candidate | pomodoro-wall-clock-timer | `endSession` suena dos veces en la transición al último bloque (preexistente)

**Detectado por**: sdd-verify en `pomodoro-wall-clock-timer`.
**Ubicación**: `src/components/CronometroPomodoro.vue` — `nextSession()`.
**Descripción**: `nextSession()` llama a `this.$playSound('endSession')` de forma incondicional al inicio del método, y la rama `else` (último bloque) tiene una segunda llamada a `this.$playSound('endSession')` antes de `running = false`. En la transición al último bloque suenan ambas, dos veces. Confirmado con `git diff c2123ab~1 c2123ab -- src/components/CronometroPomodoro.vue`: ambas líneas son contexto (sin `+`), preexistentes a este commit — no introducidas ni tocadas por la corrección de wall-clock. Fuera de alcance de `tasks.md` de este cambio (T1/T2 no lo mencionan).
**Promoción sugerida**: `sdd new fix-double-endsession-sound --domain fix` — eliminar la llamada duplicada (probablemente la del inicio del método, dejando solo la de la rama `else`, o mover la del inicio a la rama `if` para que solo suene una vez por transición intermedia también).
